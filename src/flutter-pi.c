...
static bool on_make_current(void *userdata) {
    ...
    eglMakeCurrent(flutterpi.egl.display, flutterpi.egl.surface, flutterpi.egl.surface,
                   flutterpi.egl.flutter_render_context);
    if (egl_error = eglGetError(), egl_error != EGL_SUCCESS) {
        fprintf(stderr,
                "[flutter-pi] Could not make the flutter rendering EGL context current. eglMakeCurrent: 0x%08X\n",
                egl_error);
        return false;
    }
...
static bool on_make_resource_current(void *userdata) {
    ...
    eglMakeCurrent(flutterpi.egl.display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                   flutterpi.egl.flutter_resource_uploading_context);
    if (egl_error = eglGetError(), egl_error != EGL_SUCCESS) {
        fprintf(stderr,
                "[flutter-pi] Could not make the flutter resource uploading EGL context current. eglMakeCurrent: 0x%08X\n",
                egl_error);
        return false;
    }
    ...
    if (ok == 0) {
        if (peek->state == kFramePending) {
            ...
            if (flutterpi.drm.platform_supports_get_sequence_ioctl) {
                ns = 0;
                ok = drmCrtcGetSequence(flutterpi.drm.drmdev->fd, flutterpi.drm.drmdev->selected_crtc->crtc->crtc_id,
                                        NULL, &ns);
                if (ok < 0) {
...
static int on_send_platform_message(
    ...
    if (msg->is_response) {
        result = flutterpi.flutter.libflutter_engine.FlutterEngineSendPlatformMessageResponse(flutterpi.flutter.engine,
                                                                                              msg->target_handle,
                                                                                              msg->message,
                                                                                              msg->message_size);
    } else {
...
int flutterpi_respond_to_platform_message(
    ...
    if (runs_platform_tasks_on_current_thread(NULL)) {
        ...
        if (result != kSuccess) {
            fprintf(stderr,
                    "[flutter-pi] Error sending platform message response. FlutterEngineSendPlatformMessageResponse: %s\n",
                    FLUTTER_RESULT_TO_STRING(result));
            return EIO;
        }
    } else {
...
int flutterpi_fill_view_properties(
    ...
    enum device_orientation default_orientation =
            flutterpi.display.width >= flutterpi.display.height ? kLandscapeLeft : kPortraitUp;
    ...
    if ((flutterpi.view.rotation <= 45) || ((flutterpi.view.rotation >= 135) && (flutterpi.view.rotation <= 225)) ||
        (flutterpi.view.rotation >= 315)) {
...
static int load_egl_gl_procs(void) {
    ...
    flutterpi.gl.EGLImageTargetTexture2DOES = (PFNGLEGLIMAGETARGETTEXTURE2DOESPROC) eglGetProcAddress(
            "glEGLImageTargetTexture2DOES");
    flutterpi.gl.EGLImageTargetRenderbufferStorageOES = (PFNGLEGLIMAGETARGETRENDERBUFFERSTORAGEOESPROC) eglGetProcAddress(
...
static int init_display(void) {
    ...
    {
        //just use the official 7 inch touchscreen connected via DSI
        if (connector->connector->connection == DRM_MODE_CONNECTED &&
            connector->connector->connector_type == DRM_MODE_CONNECTOR_DSI) {
    ...
    ok = drmdev_configure(flutterpi.drm.drmdev, connector->connector->connector_id, encoder->encoder->encoder_id,
                          crtc->crtc->crtc_id, mode);
    if (ok != 0) return ok;

    // only enable vsync if the kernel supplies valid vblank timestamps
    {
        uint64_t ns = 0;
        ok = drmCrtcGetSequence(flutterpi.drm.drmdev->fd, flutterpi.drm.drmdev->selected_crtc->crtc->crtc_id, NULL,
                                &ns);
        int _errno = errno;
    ...
    flutterpi.gbm.surface = gbm_surface_create_with_modifiers(flutterpi.gbm.device, flutterpi.display.width,
                                                              flutterpi.display.height, flutterpi.gbm.format,
                                                              &flutterpi.gbm.modifier, 1);
    ...
    if ((egl_error = eglGetError()) != EGL_SUCCESS) {
        fprintf(stderr,
                "[flutter-pi] Could not get the number of EGL framebuffer configurations. eglGetConfigs: 0x%08X\n",
                egl_error);
        return EIO;
    }
    ...
    if ((egl_error = eglGetError()) != EGL_SUCCESS) {
        fprintf(stderr,
                "[flutter-pi] Could not query EGL framebuffer configurations with fitting attributes. eglChooseConfig: 0x%08X\n",
                egl_error);
        return EIO;
    }
    ...
    if (_found_matching_config == false) {
        fprintf(stderr,
                "[flutter-pi] Could not find EGL framebuffer configuration with appropriate attributes & native visual ID.\n");
        return EIO;
    }
     ...
     ****************************/
    flutterpi.egl.root_context = eglCreateContext(flutterpi.egl.display, flutterpi.egl.config, EGL_NO_CONTEXT,
                                                  context_attribs);
    if ((egl_error = eglGetError()) != EGL_SUCCESS) {
    ...
    flutterpi.egl.flutter_render_context = eglCreateContext(flutterpi.egl.display, flutterpi.egl.config,
                                                            flutterpi.egl.root_context, context_attribs);
    if ((egl_error = eglGetError()) != EGL_SUCCESS) {
        fprintf(stderr,
                "[flutter-pi] Could not create OpenGL ES context for flutter rendering. eglCreateContext: 0x%08X\n",
                egl_error);
        return EIO;
    }

    flutterpi.egl.flutter_resource_uploading_context = eglCreateContext(flutterpi.egl.display, flutterpi.egl.config,
                                                                        flutterpi.egl.root_context, context_attribs);
    if ((egl_error = eglGetError()) != EGL_SUCCESS) {
        fprintf(stderr,
                "[flutter-pi] Could not create OpenGL ES context for flutter resource uploads. eglCreateContext: 0x%08X\n",
                egl_error);
        return EIO;
    }

    flutterpi.egl.compositor_context = eglCreateContext(flutterpi.egl.display, flutterpi.egl.config,
                                                        flutterpi.egl.root_context, context_attribs);
    if ((egl_error = eglGetError()) != EGL_SUCCESS) {
    ...
    flutterpi.egl.surface = eglCreateWindowSurface(flutterpi.egl.display, flutterpi.egl.config,
                                                   (EGLNativeWindowType) flutterpi.gbm.surface, NULL);
    if ((egl_error = eglGetError()) != EGL_SUCCESS) {
    ...
    if ((egl_error = eglGetError()) != EGL_SUCCESS) {
        fprintf(stderr,
                "[flutter-pi] Could not make OpenGL ES root context current to get OpenGL information. eglMakeCurrent: 0x%08X\n",
                egl_error);
        return EIO;
    }
...
static int init_application(void) {
    ...
    if (flutterpi.flutter.runtime_mode == kRelease) {
        ...
        if (libflutter_engine_handle == NULL) {
            LOG_FLUTTERPI_ERROR(
                    "[flutter-pi] Warning: Could not load libflutter_engine.so.release: %s. Trying to open libflutter_engine.so...\n",
                    dlerror());
        }
        ...
        if (libflutter_engine_handle == NULL) {
            LOG_FLUTTERPI_ERROR(
                    "[flutter-pi] Warning: Could not load libflutter_engine.so.debug: %s. Trying to open libflutter_engine.so...\n",
                    dlerror());
        }
    }
...
#	undef LOAD_LIBFLUTTER_ENGINE_PROC
    ...
    // spin up the engine
    engine_result = libflutter_engine->FlutterEngineInitialize(FLUTTER_ENGINE_VERSION, &renderer_config, &project_args,
                                                               NULL, &flutterpi.flutter.engine);
    if (engine_result != kSuccess) {
    ...
    ok = locales_add_to_fl_engine(flutterpi.locales, flutterpi.flutter.engine,
                                  libflutter_engine->FlutterEngineUpdateLocales);
    if (ok != 0) {
        return ok;
    }
...
static int init_user_input(void) {
    ...
    if (input == NULL) {
        ...
        if (ok < 0) {
            LOG_FLUTTERPI_ERROR(
                    "Couldn't listen for user input. flutter-pi will run without user input. sd_event_add_io: %s\n",
                    strerror(-ok));
...
#define PATH_EXISTS(path) (access((path),R_OK)==0)
    ...
    if (flutterpi.flutter.runtime_mode == kDebug) {
        if (!PATH_EXISTS(kernel_blob_path)) {
            fprintf(stderr,
                    "[flutter-pi] Could not find \"kernel.blob\" file inside \"%s\", which is required for debug mode.\n",
                    flutterpi.flutter.asset_bundle_path);
            return false;
        }
        ...
        if (!PATH_EXISTS(app_elf_path)) {
            fprintf(stderr,
                    "[flutter-pi] Could not find \"app.so\" file inside \"%s\", which is required for release and profile mode.\n",
                    flutterpi.flutter.asset_bundle_path);
            return false;
        }
    }
